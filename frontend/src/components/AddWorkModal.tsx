...
export function AddWorkModal({ isOpen, onClose, onImported }: AddWorkModalProps) {
      ...
      <DialogPositioner>
        <DialogContent>
          ...
          <DialogBody>
            <Stack spacing={4}>
              <FieldRoot required>
                ...
                />
                <FieldHelperText>Supports multiple URLs. Each URL will be imported in sequence.</FieldHelperText>
              </FieldRoot>
              ...
              <Box borderWidth="1px" borderColor="whiteAlpha.200" borderRadius="md" p={3}>
                ...
                <Stack spacing={2}>
                  {results.map((result, index) => (
                    <HStack key={`${result.url}-${index}`} spacing={3} align="flex-start">
                      <Icon
                        as={result.status === 'success' ? CheckCircle2 : AlertTriangle}
                        color={result.status === 'success' ? 'green.300' : 'red.300'}
                        mt="1"
                      />
                      <Box>
          ...
          <DialogFooter>
            ...
            <Button colorScheme="teal" onClick={handleSubmit} isLoading={submitting} isDisabled={!hasUrls && !submitting}>
              Import
            </Button>
          </DialogFooter>
        </DialogContent>
